...
define('OP3_SMART_THEME_VERSION', '1.0.17');
...
function zmien_continue_reading($more) {
    return str_replace('Continue Reading', 'Czytaj dalej', $more);
}
add_filter('excerpt_more', 'zmien_continue_reading');

function lista_postow_w_wierszu() {
    $args = array(
        'posts_per_page' => -1, // Wybierz, ile postów chcesz wyœwietliæ, -1 oznacza wszystkie
        'post_status'    => 'publish',
    );

    $query = new WP_Query($args);
    $output = '<div style="font-size: 12px;">'; // Zmniejszenie czcionki

    if ($query->have_posts()) {
        while ($query->have_posts()) {
            $query->the_post();
            $output .= '<span><a href="' . get_permalink() . '" style="margin-right: 15px;">' . get_the_title() . '</a></span>';
        }
    } else {
        $output .= 'Nie znaleziono postów.';
    }
...
add_shortcode('lista_postow_w_wierszu', 'lista_postow_w_wierszu');
